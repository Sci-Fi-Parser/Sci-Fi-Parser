...
def sample_style(rng: np.random.Generator, cfg: GenConfig, alias: str) -> Style:
    ...
    return Style(
        ...
        w_in=float(rng.uniform(*cfg.fig_w_in)),
        h_in=float(rng.uniform(*cfg.fig_h_in)),
        dpi=int(rng.choice(cfg.dpi_choices)),
        bar_w=float(rng.uniform(*cfg.bar_width)),
        value_lo=lo,
        value_hi=scale,
...
def series_values(rng: np.random.Generator, style: Style, n: int) -> list[np.ndarray]:
    ...
    return [np.round(rng.uniform(style.value_lo, style.value_hi, n), 2) for _ in range(style.n_series)]
