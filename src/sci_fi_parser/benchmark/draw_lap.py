...
def _dev_bell_png(devs: list[float]) -> str:
    ...
    if devs:
        ...
        nrm = n * bw * np.exp(-((xs - mu) ** 2) / (2 * sigma**2)) / (sigma * math.sqrt(2 * math.pi))
        lap = n * bw * np.exp(-np.abs(xs - med) / b) / (2 * b)
        ax.plot(
            xs, nrm, color="#94a3b8", lw=1.6, ls="--", label=f"Normal  (mean {mu:+.1f}%, sd {sd_raw:.1f})"
        )
        ax.plot(xs, lap, color="#4f46e5", lw=2.3, label=f"Laplace  (median {med:+.1f}%, b {b:.1f})")
        ...
        ax.text(
            0.5, 0.5, "no matched bars", ha="center", va="center", color="#94a3b8", transform=ax.transAxes
        )
    ...
    plt.close(fig)
    return f'<img class="devbell" src="data:image/png;base64,{base64.b64encode(buf.getvalue()).decode()}"/>'
...
def _group_table(title: str, rows: list[tuple]) -> str:
    ...
    return (
        f"<table class='full' style='max-width:560px'><thead><tr><th>{title}</th>"
...
def _type_chip(chart: dict) -> str:
    ...
    if tt is None:
        return ""
    if tp is None:
        return f" · type={html.escape(str(tt))} <span style='color:#a00'>(no pred)</span>"
    ok = "✓" if chart["type_matched"] else "✗"
    colour = "#0a0" if chart["type_matched"] else "#a00"
    return (
        f" · type=<span style='color:{colour}'>{ok}</span> "
...
def _detail_card(chart: dict, img_dir: Path) -> str:
    ...
    img_html = (
        ...
        if thumb
        else '<div style="height:120px;display:flex;align-items:center;'
...
def _summary_cards(agg: dict) -> str:
    ...
    cards: list[tuple[str, object, str]] = [
        ...
        (
            "Recall",
            f"{agg['recall'] * 100:.1f}%",
            "Fraction of true bars the extractor returned and matched. 100% = no bars missed.",
        ),
        (
            "Precision",
            f"{agg['precision'] * 100:.1f}%",
            "Fraction of predicted bars that matched a true bar. 100% = no hallucinated extras.",
        ),
        (
            "≤1%",
            f"{agg['within_1pct'] * 100:.0f}%",
            "Share of matched bars whose error is within 1% of the data range.",
        ),
        (
            "≤5%",
            f"{agg['within_5pct'] * 100:.0f}%",
        ...
        ("Total time", f"{agg['total_sec']:.1f} s", "Total wall-clock time across all charts."),
    ]
    return "".join(
...
def _pane_heading(title: str, shown: int, total: int, desc: str) -> str:
    return (
        f'<h2 title="{html.escape(desc)}" style="cursor:help">'
...
# --------------------------------------------------------------------------- #
def write_html(
    path: Path, extractor: str, agg: dict, charts: list[dict], breakdowns: list[dict], img_dir: Path
) -> None:
