...
from sci_fi_parser.benchmark.benchmark import run_benchmark
...
def _slice_profile_fields(d: dict, path: Path) -> dict:
    ...
    if unknown:
        raise ValueError(
            f"unknown key(s) in {path}: {sorted(unknown)}; valid: {sorted(profile_fields | {'name'})}"
        )
...
def _load_extends_base(raw: dict, path: Path) -> dict:
    ...
    if not ext_path.exists():
        raise ValueError(f"{path}: extends -> {ext_path} (file not found)")
...
def _parse_run_table(raw_run: dict, path: Path) -> RunConfig:
    ...
    if unknown:
        raise ValueError(f"unknown [run] key(s) in {path}: {sorted(unknown)}; valid: {sorted(_RUN_FIELDS)}")
    pull = raw_run.get("pull")
    if pull is not None and pull not in _MODE_CHOICES:
        raise ValueError(f"[run].pull in {path} must be one of {list(_MODE_CHOICES)}, got {pull!r}")
    return RunConfig(
...
def load_comparison_config(path: Path) -> tuple[RunConfig, list[CompareEntry]]:
    ...
    for raw_entry in raw.get("model", []):
        ...
        entries.append(CompareEntry(name=name, profile=VLMProfile(**{**defaults, **overrides})))
    if not entries:
...
def _ollama_request(
    method: str, path: str, body: dict | None = None, *, timeout: float = 30.0, stream: bool = False
):
...
def _pull_model(tag: str) -> None:
    ...
    resp = _ollama_request("POST", "/api/pull", {"name": tag, "stream": True}, timeout=None, stream=True)
    try:
        last_status = ""
        for raw in resp:
...
def inspect_preflight(entries: list[CompareEntry]) -> list[ModelStatus]:
    ...
    for e in entries:
        ...
        if tag in seen:
            continue
        seen[tag] = ModelStatus(tag=tag, local=tag in local, size_bytes=local.get(tag))
    return list(seen.values())


def format_preflight(statuses: list[ModelStatus], mode: str | None = None) -> str:
    ...
    lines.append(f"  free disk:   {_human_bytes(free)}  ({_ollama_models_dir()})")
    if mode:
...
def _prompt_mode(missing: list[str]) -> str:
    ...
    if not _is_interactive():
        raise SystemExit(
            f"missing models {missing} but stdin is not a TTY. Pass --pull {{prefetch,circular,skip}}."
        )
...
def _confirm_yn(prompt: str) -> bool:
    if not _is_interactive():
        raise SystemExit(f"{prompt} -- not a TTY; pass --yes to confirm non-interactively.")
...
def _row_dict(name: str, tag: str, agg: dict | None, error: str | None) -> dict:
...
def _html_row(row: dict) -> str:
    ...
    if "error" in row:
        cells = [
            f"<td>{link}</td>",
            f"<td><code>{row['tag']}</code></td>",
            f"<td colspan='{len(_LEADER_COLS) - 2}' style='color:#a00'>"
            f"ERROR: {html.escape(row['error'])}</td>",
        ]
    else:
        cells = [_html_cell(key, link, row, fmt) for _, key, fmt in _LEADER_COLS]
...
# --------------------------------------------------------------------------- #
def _safe_run(
    entry: CompareEntry, data: Path, out: Path, seed: int, limit: int | None
...
def resolve_pull_mode(missing: list[str], pull: str | None, assume_yes: bool) -> str:
    ...
    if assume_yes:
        raise SystemExit(
            "missing models present and --yes given but no --pull mode; pick --pull {prefetch,circular,skip}."
        )
...
def _maybe_drop(tag: str, mode: str, pulled_by_us: set[str], still_needed: set[str], local: set[str]) -> None:
...
def _maybe_resume_row(entry: CompareEntry, out: Path, resume: bool) -> dict | None:
...
def _process_entry(
    ...
    if tag not in local and mode == "skip":
        return _row_dict(entry.name, tag, None, f"model {tag!r} not pulled (mode=skip)")
    if mode == "circular":
...
def _run_entries(
    ...
    for i, entry in enumerate(entries, 1):
        ...
        row = _process_entry(entry, data, out, seed, limit, mode, local, pulled_by_us, resume)
        rows.append(row)
        write_leaderboard(rows, out)
        still_needed = {e.profile.model for e in entries[i:]}
        _maybe_drop(entry.profile.model, mode, pulled_by_us, still_needed, local)
    return rows
...
def _merge_run(
    ...
    *,
    ...
    limit: int | None,
    pull: str | None,
) -> tuple[Path, Path, int, int | None, str | None]:
    ...
    if data_v is None:
        raise SystemExit("no dataset specified: pass --data or set [run].data in the TOML")
...
def run_comparison(
    ...
    data, out, seed, limit, pull = _merge_run(run_cfg, data=data, out=out, seed=seed, limit=limit, pull=pull)
    ...
    rows = _run_entries(entries, data, out, seed, limit, mode, local, resume=resume)
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        type=Path,
        default=None,
        help="synthetic dataset dir (images/ + labels.jsonl); overrides [run].data in the TOML",
    )
    ap.add_argument(
        "--config", type=Path, default=Path("config/vlm_comparison.toml"), help="comparison config TOML"
    )
    ap.add_argument(
        "--out",
        type=Path,
        default=None,
        help="parent dir; each model gets a subdir + a leaderboard.* is written here (overrides [run].out)",
    )
    ap.add_argument("--seed", type=int, default=None, help="overrides [run].seed (default: 0)")
    ap.add_argument(
        "--limit", type=int, default=None, help="cap each model to the first N images (overrides [run].limit)"
    )
    ap.add_argument(
        "--pull",
    ...
    ap.add_argument(
        ...
        dest="assume_yes",
        action="store_true",
        help="skip the 'this will download N model(s)' prompt; requires --pull when models are missing",
    )
    ap.add_argument("--dry-run", action="store_true", help="print the preflight summary and exit")
    ap.add_argument(
        "--resume",
        action="store_true",
        help="skip any model whose <out>/<name>/results.json already exists; useful after a crash",
    )
    args = ap.parse_args()
    run_comparison(
        args.config,
