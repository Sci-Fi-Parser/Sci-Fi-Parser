...
def score_chart_values(
    ...
    matched_pkeys = {pnorm[benchmark.normalize_key(*k)] for k in tmap
                     if benchmark.normalize_key(*k) in pnorm}
    ...
    result = benchmark.ChartResult(
        image_id, len(tmap), len(pmap), matched, len(tmap) - matched, extra, errors,
...
def score_vlm_outputs(
    ...
    for image_id, record in image_set.items():
        ...
        results.append(score_chart_values(
